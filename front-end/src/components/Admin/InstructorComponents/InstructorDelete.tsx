...
import type { InstructorDeleteProps } from '../interface/instructor';
...
const InstructorDelete = ({
	...
	instructor_id,
	onSuccess,
}: InstructorDeleteProps) => {
	const [loading, setLoading] = useState(false);
	const handleDelete = async () => {
		if (!instructor_id) return;
		setLoading(true);
		try {
			const res = await fetch(`${apiUrl}/instructors/delete/${instructor_id}`, {
				method: 'DELETE',
				headers: {
			...
			if (!res.ok) {
				toast.error('Failed to delete the instructor');
				return;
			}
