...
import type { RescheduleConsultationprops } from '../interface/consultation';
...
const RescheduleConsultation = ({
	...
	const filterWeeklies = weeklies.filter(
		(w) => w.adviser_id == selectedSchedule?.adviser_id,
	);

	const getStudentIds: number[] =
		selectedSchedule.student.proponent_detail.proponent.details.map(
			(d) => d.student_id,
		);
	const checkExistigSchedule = (checkDate: Date): boolean => {
		...
		const checkExistingDate = filterWeeklies.filter(
			(s) =>
				...
				getStudentIds.includes(s.student_id),
		);
	...
	const handleSubmit = async () => {
		...
		const payLoad = {
			adviser_id: selectedSchedule?.adviser_id,
						...
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<p className="text-sm text-muted-foreground">
									with{' '}
									{selectedSchedule.student.proponent_detail.proponent.title}
								</p>
							</div>
						</div>
					</AlertDialogHeader>
						...
						{currentStep === 'window' && (
							<div className="space-y-4">
								<div>
									...
									<p className="text-sm text-muted-foreground">
										Choose from {selectedSchedule.adviser.name}'s available time
										...
										.map((window) => {
											...
											return (
												<button
													...
													className={cn(
														'p-4 rounded-lg border-2 text-left transition-all cursor-pointer disabled:cursor-auto disabled:border-none disabled:bg-muted disabled:text-slate-600! hover:disabled:border-none relative hover:disabled:bg-muted',
